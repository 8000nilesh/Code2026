...
def upload_and_register_evidence(
    ...
    canonical_drive_file_id = upload_res.get("drive_file_id") or upload_res.get("file_id")
    # If gdrive_web_view_link already exists, use it directly without unnecessarily reconstructing URL
    web_view_link = upload_res.get("gdrive_web_view_link") or upload_res.get("web_view_link")
    if not web_view_link and canonical_drive_file_id:
        web_view_link = f"https://drive.google.com/file/d/{canonical_drive_file_id}/view"

    doc_filename = upload_res.get("filename") or upload_res.get("file_name")

    cur.execute("""
        INSERT INTO evidence_documents (
            document_id, project_id, parcel_id, document_type,
            filename, file_name, file_size_bytes, mime_type, sha256_hash,
            storage_provider, drive_file_id, gdrive_file_id, gdrive_web_view_link,
            gdrive_download_link, bitemporal_event_id, created_at, uploaded_at
        ) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, NOW(), NOW())
        ON CONFLICT (sha256_hash) DO UPDATE SET
            parcel_id = EXCLUDED.parcel_id,
            drive_file_id = EXCLUDED.drive_file_id,
            gdrive_file_id = EXCLUDED.gdrive_file_id,
            gdrive_web_view_link = COALESCE(EXCLUDED.gdrive_web_view_link, evidence_documents.gdrive_web_view_link);
    """, (
        doc_id, project_id, parcel_id, doc_type,
        doc_filename, doc_filename, upload_res['file_size_bytes'], upload_res['mime_type'],
        upload_res['sha256_hash'], 'GOOGLE_DRIVE', canonical_drive_file_id, canonical_drive_file_id,
        web_view_link, upload_res.get('download_link'), event_id
    ))
    print(f"      [SUCCESS] Document registered in Neon DB table 'evidence_documents' (Doc ID: {doc_id}, Drive ID: {canonical_drive_file_id})")
    ...
    """, (
        ...
        json.dumps({
            ...
            "gdrive_file_id": canonical_drive_file_id,
