...
class GDriveClient:
    ...
    def _authenticate_api(self):
        ...
        try:
            ...
            print(f"[WARN] Google Drive client library missing ({e}). Running in offline mode.")
            self.mode = "OFFLINE"
            return
        ...
        if not creds or not creds.valid:
            if creds and creds.expired and creds.refresh_token:
                ...
                if not Path(client_secret).exists():
                    print("[INFO] No Google Drive credentials found. Running in offline mode.")
                    self.mode = "OFFLINE"
                    return
                try:
                    ...
                    print("[WARN] google_auth_oauthlib not installed. Running in offline mode.")
                    self.mode = "OFFLINE"
                    return
    ...
    def upload_evidence_file(
        ...
        if self.mode == "WEBHOOK":
            ...
            drive_id = res_json.get("file_id")
            web_link = res_json.get("web_link") or f"https://drive.google.com/file/d/{drive_id}/view"
            return {
        ...
        web_link = uploaded.get('webViewLink') or f"https://drive.google.com/file/d/{drive_id}/view"
